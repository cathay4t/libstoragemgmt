...
START_TEST(test_local_disk_smart_status_get)
{
    int rc = 0;
    int32_t smart_status = LSM_DISK_SMART_STATUS_UNKNOWN;
    lsm_error *lsm_err = NULL;

    rc = lsm_local_disk_smart_status_get("/dev/sda", &smart_status,  &lsm_err);
    if (rc == LSM_ERR_OK) {
        fail_unless(smart_status == LSM_DISK_SMART_STATUS_UNKNOWN,
                    "lsm_local_disk_smart_status_get(): "
                    "Expecting smart_status to be "
                    "LSM_DISK_SMART_STATUS_UNKNOWN "
    ...
    rc = lsm_local_disk_smart_status_get(NOT_EXIST_SD_PATH, &smart_status,
                                         &lsm_err);
    fail_unless(rc == LSM_ERR_NOT_FOUND_DISK,
                "lsm_local_disk_smart_status_get(): "
                "Expecting LSM_ERR_NOT_FOUND_DISK error with "
                "non-existent sd_path");
    fail_unless(lsm_err != NULL, "lsm_local_disk_smart_status_get(): "
...
Suite * lsm_suite(void)
{
    ...
    tcase_add_test(basic, test_local_disk_smart_status_get);
