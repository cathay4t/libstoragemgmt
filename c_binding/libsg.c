...
#define _T10_SPC_INFO_EXCEPTION_PAGE_CODE                    0x2f
...
#define INFO_EXCEPTION_CONTROL_PAGE                          0x1c

/* Found documentation via Seagate SCSI Commands Reference Manual */
#define MRIE_REPORT_INFO_EXCEPTION_ON_REQUEST                0x6
...
struct _sg_t10_informational_exceptions_mode_page_0_hdr {
    uint8_t dont_care[3];
    uint8_t reserved : 4;
    uint8_t mrie : 4;
};

struct _sg_t10_informational_exceptions_general_log_hdr {
    uint8_t dont_care[4];
    uint8_t asc;
    uint8_t ascq;
};
...
int32_t _sg_smart_interpret_asc(uint8_t asc)
{
    if (asc == _T10_SPC_ASC_IMPENDING_FAILURE)
        return LSM_DISK_SMART_STATUS_FAIL;
    else if (asc == _T10_SPC_ASC_WARNING)
        return LSM_DISK_SMART_STATUS_WARN;
    else
        return LSM_DISK_SMART_STATUS_GOOD;
}

/*
 * Query a SAS drive to get its SMART Status.
 ...
 * 3. Return the SMART Status to the caller.
 * 
 * Input *smart_status should be a pointer to an int32_t value.
 * Return LSM_ERR_NO_MEMORY or LSM_ERR_NO_SUPPORT or LSM_ERR_LIB_BUG or
 * LSM_ERR_NOT_FOUND_DISK.
 */
int _sg_sas_smart_status(char *err_msg, int fd, int32_t *smart_status)
{
    int rc = LSM_ERR_OK;
    uint8_t informational_exceptions_mode_page[_SG_T10_SPC_MODE_SENSE_MAX_LEN];
    uint8_t informational_exceptions_log_page[_T10_SPC_LOG_SENSE_MAX_LEN];
    uint8_t asc = 0;
    uint8_t requested_sense[_T10_SPC_SENSE_DATA_MAX_LENGTH];
    struct _sg_t10_sense_fixed *sense_fixed = NULL;
    struct _sg_t10_informational_exceptions_mode_page_0_hdr *ie_mode_hdr = NULL;
    struct _sg_t10_informational_exceptions_general_log_hdr *ie_log_hdr = NULL;

    _good(_sg_io_mode_sense(err_msg, fd, INFO_EXCEPTION_CONTROL_PAGE, 0,
                            informational_exceptions_mode_page),
          rc, out);
    ie_mode_hdr = (struct _sg_t10_informational_exceptions_mode_page_0_hdr *)
                  informational_exceptions_mode_page;
    
    if (ie_mode_hdr->mrie == MRIE_REPORT_INFO_EXCEPTION_ON_REQUEST) {
        _good(_sg_request_sense(err_msg, fd, requested_sense), rc, out);
        sense_fixed = (struct _sg_t10_sense_fixed *) requested_sense;
        asc = sense_fixed->asc;
    } else {
        _good(_sg_log_sense(err_msg, fd, _T10_SPC_INFO_EXCEPTION_PAGE_CODE, 0,
                            informational_exceptions_log_page), rc, out);
        // SPC5 rev07 - Table 349 - Informational Exceptions General log parameter
        ie_log_hdr = (struct _sg_t10_informational_exceptions_general_log_hdr *)        
                     informational_exceptions_log_page;
        asc = ie_log_hdr->asc;
    }

    *smart_status = _sg_smart_interpret_asc(asc);
...
/*
 * Query a SATA drive attached via SAS to get its SMART Status.
 * 
 * Input *smart_status should be a pointer to an int32_t value.
 * Return LSM_ERR_NO_MEMORY or LSM_ERR_NO_SUPPORT or LSM_ERR_LIB_BUG or
 * LSM_ERR_NOT_FOUND_DISK.
 */
int _sg_ata_passthrough_smart_status(char *err_msg, int fd,
                                     int32_t *smart_status)
{
    int rc = LSM_ERR_OK;
    ...
    if (!rc)
        *smart_status = _ata_smart_status_interpret_output_regs(ata_output_regs);
