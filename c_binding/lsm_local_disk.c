 ...
 *  * Request SMART data the appropriate way.
 */
int lsm_local_disk_smart_status_get(const char *disk_path,
                                    ...
                                    lsm_error **lsm_err)
{
    ...
    _good(_check_null_ptr(err_msg, 3, disk_path, smart_status, lsm_err),
          rc, out);
    ...
    *lsm_err = NULL;
    ...
    if (link_type == LSM_DISK_LINK_TYPE_ATA) {
        _good(_sg_ata_passthrough_smart_status(err_msg, fd, smart_status),
              rc, out);
    } else if (link_type == LSM_DISK_LINK_TYPE_SAS) {
        _good(_sg_sas_smart_status(err_msg, fd, smart_status), rc, out);
    } else {
        rc = LSM_ERR_NO_SUPPORT;
...
out:
    if (rc != LSM_ERR_OK) {
        ...
        if (smart_status != NULL)
            *smart_status = LSM_DISK_SMART_STATUS_UNKNOWN;
    }
